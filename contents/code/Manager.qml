...
Item {
  ...
  function ignored(client) {
    return client.transient || config.type.every(t => !client[t]) || (config.ignored.caption && config.ignored.caption.test(client.caption)) || (config.ignored.name && config.ignored.name.test(client.name));
  }
